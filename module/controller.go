...
func GCFPostHandler(PASETOPRIVATEKEYENV, collectionname string, r *http.Request) string {
	...
	user, err := LogIn(MongoConnect(), collectionname, datauser)
	if err != nil {
		Response.Message = err.Error()
		return GCFReturnStruct(Response)
	}
	Response.Status = true
	tokenstring, err := Encode(user.ID, user.FirstName, user.LastName, user.Email, user.Role, os.Getenv(PASETOPRIVATEKEYENV))
	if err != nil {
		Response.Message = "Gagal Encode Token : " + err.Error()
	} else {
		Response.Message = "Selamat Datang " + user.FirstName + " " + user.LastName
...
func Encode(id primitive.ObjectID, firstname, lastname, email, role string, privateKey string) (string, error) {
	...
	// token.Set("id", id)
	token.SetString("id", id.Hex())
	token.SetString("firstname", firstname)
	token.SetString("lastname", lastname)
